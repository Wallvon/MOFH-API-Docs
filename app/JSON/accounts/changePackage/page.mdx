---
title: Change package
---

import { Callout } from "nextra/components";
import { Tabs } from "nextra/components";

# Change package

This endpoint changes the package of the vPanel user account associated with provided information.

## HTTP Endpoint

`POST https://panel.myownfreehost.net/json-api/changepackage.php`

## Usage

<Callout emoji="⚠️" type="warning">
  Your package name must be fully lowercase since otherwise the user will get
  the "Client is not allocated to a package" package which has the maximum
  resources.
</Callout>

<Tabs items={['cURL', 'Python']}>
    <Tabs.Tab>
        ```bash
        curl -X POST -u username:password -d "user=example&pkg=myawesomeplan" "https://panel.myownfreehost.net/json-api/changepackage.php"
        ```
    </Tabs.Tab>
    <Tabs.Tab>
        ```python
        import requests
        from requests.auth import HTTPBasicAuth
        # iFastNet decided it was a good idea to return XML on a JSON API endpoint.
        from xml.etree.ElementTree import fromstring, ElementTree # Do this if you only want to get the status code.

        url = "https://panel.myownfreehost.net/json-api/changepackage.php"
        data = {'user': 'username', 'pkg': 'myawesomeplan'} # Keep in mind that the package name must be fully lowercase.

        response = requests.post(url, params=data, auth=HTTPBasicAuth('username', 'password'))

        print(response.text)

        # Do this if you only want to get the status code.
        tree = ElementTree(fromstring(response.content))
        root = tree.getroot()
        for child in root:
            print(child[1].text) # The status code (can either be 0 or 1)
        ```
    </Tabs.Tab>

</Tabs>

## Response

<Callout emoji="⚠️" type="warning">
  For some reason iFastNet decided it was a good idea to return XML on a JSON
  API endpoint. This is why the response is in XML.
</Callout>

<Callout emoji="💡">
    **The above request returns XML structured like the following:**

    ```xml
    <changepackage>
        <result>
            <rawout>
            </rawout>
            <status>1</status>
            <statusmsg>Account Upgrade/Downgrade Complete for asdfasdf</statusmsg>
        </result>
    </changepackage>
    ```

</Callout>
