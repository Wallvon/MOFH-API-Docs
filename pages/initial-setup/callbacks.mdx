...
# Callbacks
The MyOwnFreeHost API allows you to set up a callback URL that will be called when a user account gets activated, suspended or unsuspended, or when an SQL cluster gets added to a user. The callback URL is also used for the ticketing system
...
<Callout emoji="ℹ️" type="info">
    **What are the domain codes?**
    ...
    Parked Domain Removed:
    - `CLIENTPARKREM`

    Subdomain Removed:
    - `CLIENTSUBREM`

    Addon Domain Removed:
    - `CLIENTDOMREM`
</Callout>
